...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        #from https://github.com/vinuni-vishc/Few-Shot-Cosine-Transformer/blob/main/methods/transformer.py#L73
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.obj_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        self.pair_token = nn.Parameter(torch.zeros(1, 1, embed_dim))
        ...
        trunc_normal_(self.obj_token, std=0.02)
        trunc_normal_(self.pair_token, std=0.02)
    ...
    def no_weight_decay(self):
        return {"pos_embed", "cls_token","obj_token","pair_token"}
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False):
        ...
        select = torch.cat(select, dim=0).to(torch.device('cuda'))
        ...
        obj_tokens = self.obj_token.expand(B, -1, -1)
        pair_tokens = self.obj_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens,obj_tokens,pair_tokens, x), dim=1)
        pos_embed = torch.cat(
            (self.pos_embed[:, 0, :][:, None, :].expand(B, -1, -1), self.pos_embed[:, 1, :][:, None, :].expand(B, -1, -1), self.pos_embed[:, 2, :][:, None, :].expand(B, -1, -1),pos_embed), dim=1)
        ...
        x_mask = torch.cat([torch.ones(x_mask.shape[0], 1).to(x_mask), torch.ones(x_mask.shape[0], 1).to(x_mask), torch.ones(x_mask.shape[0], 1).to(x_mask), x_mask], dim=1)
...
def checkpoint_filter_fn(state_dict, model):
    ...
    for k, v in state_dict.items():
        if "patch_embed.proj.weight" in k and len(v.shape) < 4:
            ...
            v = resize_pos_embed(v, model.pos_embed, 3)
            
        out_dict[k] = v
    return out_dict

#create transformer one with urls
# def _create_vision_transformer(variant, pretrained=True, distilled=False, **kwargs):
